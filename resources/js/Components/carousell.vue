...
<template>
            ...
            <div class="relative w-screen h-[40vh] md:h-[90vh] overflow-hidden">
                <div class="flex w-full h-full transition-transform duration-700"
                    ...
                    <div v-for="(slide, index) in slides" :key="index" class="min-w-full relative h-full">
                        ...
                        <div class="absolute inset-0 bg-black/40 flex flex-col items-center justify-center text-white text-center px-6 md:px-12">
                            <h2 class="text-3xl md:text-5xl font-bold drop-shadow-lg">{{ slide.title }}</h2>
                            <a :href="slide.link" class="mt-4 px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white text-lg font-semibold rounded-full shadow-md hover:scale-105 transition-transform">
                                {{ slide.buttonText }}
                            </a>
                        </div>
                    </div>
    ...
    @click="prevSlide">
    <span class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-black/50 hover:bg-black/70">
