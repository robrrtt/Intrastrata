...
const slides = ref([
    { image: '/img/nyc.jpg', title: 'Fire & Allied Perils', details: 'Protect your home or business from fire, natural calamities, and more.', buttonText: 'Learn More', link: "/products/Fire" },
        ...
        <div class="overflow-hidden bg-white shadow-sm">
                    ...
                    <div v-for="(slide, index) in slides" :key="index" class="min-w-full relative h-full">
                        ...
                        <div
                            ...
                            <h1
                                ...
                                {{ slide.title }}</h1>
                            <p
                                class="mb-6 text-lg font-normal text-white-500 lg:text-xl sm:px-16 xl:px-48 dark:text-white-400">
                                {{ slide.details }}</p>
            ...
            <div class="absolute z-30 flex space-x-2 bottom-5 left-1/2 transform -translate-x-1/2">
                <button v-for="(slide, index) in slides" :key="index" @click="goToSlide(index)"
                    ...
                    :class="currentSlide === index ? 'bg-orange-500 scale-125' : 'bg-gray-300'">
                </button>
            </div>

            <button class="absolute top-0 left-0 z-30 flex items-center justify-center h-full px-5 md:px-20"  @click="prevSlide">
                <span class="group inline-flex items-center justify-center w-12 h-12 rounded-full bg-white/10 backdrop-blur-md shadow-lg transition duration-300 ease-in-out hover:bg-orange-400 hover:shadow-xl">
                    <svg class="w-3 h-3 text-white group-hover:scale-110 transition-transform duration-300" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 1 1 5l4 4" />
                    </svg>
                </span>
            </button>

            <button class="absolute top-0 right-0 z-30 flex items-center justify-center h-full px-5 md:px-20"  @click="nextSlide">
                <span class="group inline-flex items-center justify-center w-12 h-12 rounded-full bg-white/10 backdrop-blur-md shadow-lg transition duration-300 ease-in-out hover:bg-orange-400 hover:shadow-xl">
                    <svg class="w-3 h-3 text-white group-hover:scale-110 transition-transform duration-300" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
